 ...
 */
export async function validateModelDirectory(directory, entry, overrides = {}) {
    ...
    const issue = (code, resource, message) => {
    ...
    const read = async (relative, maxBytes) => {
        if (!safeRelative(relative)) {
            issue('invalid-path', relative, '资源路径不能包含绝对路径、URL、编码路径或上级目录。');
            return;
        }
        let handle;
        try {
            ...
            issue(code === 'ENOENT'
                ...
                : code === 'ELOOP'
                    ? 'symlink'
                    : 'read-failed', relative, code === 'ENOENT' ? '找不到引用的资源。' : '无法安全读取资源。');
            return;
        }
        finally {
            await handle?.close();
    ...
    const manifestBytes = await read(entry, Math.min(limits.manifestBytes, limits.fileBytes));
    if (!manifestBytes)
        return result;
    ...
    const references = [];
    const countedReferences = new Set([entry]);
    let referenceCount = 1;
    const add = (value, kind, suffix) => {
    ...
    if (!Array.isArray(refs.Textures) || !refs.Textures.length) {
        ...
        refs.Textures.forEach((value) => add(value, 'texture', '.png'));
    ...
    for (const ref of references) {
        ...
        if (!ref.value.endsWith(ref.suffix)) {
            issue('unsupported-resource', ref.value, `该资源必须使用 ${ref.suffix} 格式。`);
            continue;
        }
        ...
        if (seen.has(relative))
            continue;
        seen.add(relative);
        const bytes = await read(relative, ref.suffix.endsWith('.json')
        ...
        if (ref.kind === 'texture') {
            try {
                ...
                issue('invalid-resource', relative, 'PNG 损坏或超过像素上限；仅支持 8-bit 非隔行灰度/RGB/RGBA，暂不支持索引色。');
            }
        }
        else if (ref.kind === 'moc') {
            if (bytes.length < 8 || bytes.toString('ascii', 0, 4) !== 'MOC3')
                issue('invalid-resource', relative, '缺少 MOC3 文件头；完整兼容性仍需 Cubism 校验。');
        }
...
function validatePng(bytes, limits) {
    ...
    if (!bytes.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])))
        fail();
    let offset = 8, expected = 0, rowBytes = 0, height = 0, ended = false, idatEnded = false;
    const data = [];
    while (offset + 12 <= bytes.length) {
        ...
        if (offset === 8) {
            ...
            const channels = { 0: 1, 2: 3, 4: 2, 6: 4 }[payload[9]];
            if (!width ||
                !height ||
            ...
            if (!/^[a-z]/u.test(type) && type !== 'PLTE')
                fail();
        }
        offset += 12 + length;
    }
    if (!ended || !data.length || !expected)
        fail();

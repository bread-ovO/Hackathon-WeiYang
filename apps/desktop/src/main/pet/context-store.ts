...
export function createPetContextStore(file: string) {
  return {
    async load(): Promise<ContextStoreState | null> {
      try {
        const stat = await lstat(file)
        if (!stat.isFile() || stat.isSymbolicLink() || stat.size > 16384)
          throw new Error('PET_CONTEXT_STORAGE_ERROR')
        ...
        try {
          ...
          if (total > 16384) throw new Error('PET_CONTEXT_STORAGE_ERROR')
          return parseContextStoreState(
    ...
    async save(value: ContextStoreState): Promise<void> {
      const text = JSON.stringify(parseContextStoreState(value))

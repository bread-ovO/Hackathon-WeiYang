...
import { DEFAULT_MODEL_LIMITS, validateModelDirectory, } from './model-validation';
...
const safeRelative = (value) => value.length > 0 &&
    value.length <= 1024 &&
...
function canonicalLocation(directory) {
...
const digest = (entry, resources) => createHash('sha256')
...
export class ModelStore {
    ...
    async importModel(sourceDirectory, entry) {
        return serial(this.root, async () => {
            ...
            if (source === this.root ||
                ...
                throw new ModelStoreError('invalid-store', '源模型目录与受控存储目录不能重叠。');
            }
            const id = digest(entry, validation.resources);
            const existing = state.models.find((model) => model.id === id);
            if (existing)
                return { status: 'duplicate', model: clone(existing) };
            if (state.models.length >= maxModels)
                throw new ModelStoreError('storage-limit', '最多保存 64 个模型，请先移除不再使用的模型。');
            ...
            try {
                ...
                if (!copied.ok || digest(entry, copied.resources) !== id)
                    throw new ModelStoreError('source-changed', '复制后的模型与已校验资源不一致，未导入。');
    ...
    async initialize() {
        ...
        if (!info.isDirectory() ||
            ...
            throw new ModelStoreError('invalid-store', '存储目录不能通过符号链接访问。');
        if (process.platform !== 'win32' && (info.mode & 0o077) !== 0)
            throw new ModelStoreError('invalid-store', '存储目录必须是仅当前用户可访问的私有目录。');
    }
    async readState() {
        ...
        try {
            ...
            if (!validState(value))
                throw new Error('registry-shape');
            return value;
        }
        catch {
            throw new ModelStoreError('invalid-store', '模型注册表损坏；保留原数据，未重置。');
        }
        finally {
            await file.close();
        }
...
function validState(value) {
    ...
    for (const model of state.models) {
        ...
        if (digest(model.entry, model.resources) !== model.id ||
            model.totalBytes !==
...
async function readSourceSnapshot(root, resource) {
    const changed = () => new ModelStoreError('source-changed', '源资源在校验后发生变化，请重新导入。');
    const parents = [];
    let target = root;
...
async function syncDirectory(directory) {
    if (process.platform === 'win32')
        return; // Windows directory fsync unsupported; rename remains atomic.
...
async function syncTreeDirectories(directory) {

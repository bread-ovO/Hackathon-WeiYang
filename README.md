...
「设置 → AI 模型」支持配置 **OpenAI Responses API / Chat Completions API**，也可调用本机已登录的 **Codex / Claude Code CLI**。配置 Base URL、模型名称及加密密钥引用，或选择 CLI 并启用分析授权。在「跟进 → AI 整理」点击开始整理才发送所选会话的有限上下文；确认后加入跟进，模型不会直接完成事项。详情及实测范围见 [模型服务接入](docs/engineering/模型服务接入_2026-09-14.md)。

当前为用户主动触发的有限上下文分析，尚未替代后台全历史规则处理，跨会话及跨渠道自动归并仍待接通。历史本地模型评测（不是当前默认接入方式）与限制见 [AI 事项分析验收](docs/evals/AI事项分析验收.md)。

## AI 任务聊天与编排

左侧「AI 聊天」选择项目后，可自然语言查询、新增、修改任务，或删除到可恢复回收站。AI 先展示变更预览，确认后原子提交，保护人工修改与重复确认。模型沿用 Responses / Chat Completions 或已登录 CLI 配置；处理过程默认折叠。跟进页的 AI 整理支持已同步、授权有效的飞书会话。

详见 [Agent 编排与任务聊天](docs/engineering/Agent编排与任务聊天_2026-09-14.md)。本轮为受控查询工具循环与确认写入；自动多源全历史调度和完整并发 DAG 仍待实现。
...
- SQLite v1–v24 显式迁移、WAL/FULL、外键、事件/作业/游标事务和修订去重。
...
- 事项时间线：分页追溯人工调整、规则候选、引用冲突、版本确认与撤回影响；刷新保留编辑草稿，见 [时间线与冲突审计](docs/engineering/事项时间线与引用冲突审计_2026-09-13.md)。
...
已支持用户选择本地 JSONL 导出文件、按项目导入、增量同步和撤销授权；GitHub 已支持限定仓库令牌与持久轮询；飞书已支持手动令牌与限定会话的历史及增量采集；Claude Code/Codex 的受支持 JSONL 会话格式已接入，已接入手动触发的单会话模型分析；“示例体验”提供虚构事项，支持筛选、搜索、新增及可撤销的状态预览，仅保存在窗口内存；“我的工作区”已支持真实项目、手动事项、标题与状态修改、归档及重启恢复。已接通本机有限规则的后台候选整理，支持暂停和原文依据；后台全历史模型语义处理、自动条件核验、飞书令牌自动刷新及加密数据库仍待开发。
...
队列存储与故障恢复说明见 [S05 交付记录](docs/engineering/S05_持久作业与租约恢复_2026-09-13.md)。作业处理器已接本地明确承诺规则，只生成待确认候选。后台自动消费者尚未连接语义模型；不把队列领取等同于事项处理成功。

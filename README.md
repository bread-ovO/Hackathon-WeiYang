...
- SQLite v1–v5 显式迁移、WAL/FULL、外键、事件/作业/游标事务和修订去重。
...
已支持用户选择本地 JSONL 导出文件、按项目导入、增量同步和撤销授权；飞书、原生 AI 会话格式、GitHub、模型与 HTTP 插件执行尚未接入；“示例体验”提供虚构事项，支持筛选、搜索、新增及可撤销的状态预览，仅保存在窗口内存；“我的工作区”已支持真实项目、手动事项、标题与状态修改、归档及重启恢复。自动条件核验、后台消费器、自动来源采集及加密数据库仍待开发。

## 桌宠

用户可导入自有 Live2D Cubism 运行时模型（`.model3.json` 目录，不接受 `.cmo3` 工程文件），经只读资源预检与受控 staging 复制后，在独立透明无边框窗口中由 Cubism SDK 实时渲染：待机动作循环、呼吸与眨眼，缺失可选动作时降级为纯呼吸模式，坏 moc3 诚实回落占位图形。窗口支持拖动、滚轮缩放（0.5–2 倍）、位置持久恢复与多屏回收；透明区域鼠标穿透，帧率闲置自动降档、隐藏暂停、WebGL 上下文丢失自恢复。桌宠以低频文字气泡主动说话（默认随机 45–90 分钟、每日上限 6 句、22:00–09:00 静默、锁屏抑制、唤醒不补发），频率/暂停/静默时段可在设置中调整并持久化；气泡不抢键盘焦点，TTS 与口型联动仍为后续增强。默认不主动播音，不根据沉默推断事项完成。模型资产永不进入仓库，许可边界见工程文档。

**实测状态（2026-09-13）**：PET01 SDK 兼容性验证、PET02 导入入口、PET05–08 窗口机制、PET06 真实渲染、PET09–11 主动说话、PET14 性能与恢复、PET15 许可清单与 CI 资产守卫、PET16 集成验收均已交付（Windows x64 本机实测 + macOS CI；渲染/演示场景需本地 PET01 资产，CI 显式 skip）。PET12/PET13（P1：可解释话语、语音口型）未开工。

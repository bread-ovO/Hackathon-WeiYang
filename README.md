...
- SQLite v1–v3 显式迁移、WAL/FULL、外键、事件/作业/游标事务和修订去重。
...
飞书、AI 会话、GitHub、模型与插件执行尚未接入；“示例体验”提供虚构事项，支持筛选、搜索、新增及可撤销的状态预览，仅保存在窗口内存；“我的工作区”已支持真实项目、手动事项、标题与状态修改、归档及重启恢复。自动条件核验、后台消费器、来源采集及加密数据库仍待开发。
...
目前自动事件接收仅供集成测试；调用方须先授权并登记来源，同一来源分页串行提交。数据库登记不代替正式授权模块。迁移 v1 建立最小表结构，v2 增加可重建候选检索索引；v3 增加项目、条件版本、证据关系、人工决定/修订和待发送 outbox；没有后台消费器或通知配送。
...
真实工作区与第二批实现边界见[交付记录](docs/engineering/真实工作区与模型导入_2026-09-13.md)。当前真实事项列表每项目最多显示 100 条；来源/活跃度筛选、条件与截止编辑、人工撤销仍待开发。

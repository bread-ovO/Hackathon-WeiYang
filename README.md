...
- SQLite v1–v4 显式迁移、WAL/FULL、外键、事件/作业/游标事务和修订去重。
...
| packages/plugin-host | 版本化声明式 manifest 校验，目前不执行插件 |
...
目前自动事件接收仅供集成测试；调用方须先授权并登记来源，同一来源分页串行提交。数据库登记不代替正式授权模块。迁移 v1 建立最小表结构，v2 增加可重建候选检索索引；v3 增加项目、条件版本、证据关系、人工决定/修订和待发送 outbox，v4 增加截止时间及全事项列表检索索引；没有后台消费器或通知配送。

...
连接页顶部的「AI 事项分析」可对一个已授权本机会话运行 **Ollama / Qwen2.5 7B**，生成事项标题、建议阶段、下一步与原文依据。用户确认后进入跟进清单；模型不会直接完成事项。先执行 `ollama pull qwen2.5:7b` 并运行本机 Ollama 服务。

当前为用户主动触发的有限上下文分析，尚未替代后台全历史规则处理，跨会话及跨渠道自动归并仍待接通。协议测试、真实本地模型回归和限制见 [AI 事项分析验收](docs/evals/AI事项分析验收.md)。

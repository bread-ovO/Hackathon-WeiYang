...
连接页顶部的「AI 事项分析 → 分析模型」支持配置 **OpenAI Responses API / Chat Completions API**，也可调用本机已登录的 **Codex / Claude Code CLI**。配置 Base URL、模型名称及加密密钥引用，或选择 CLI 并启用分析授权。点击分析才发送所选会话的有限上下文；确认后加入跟进，模型不会直接完成事项。详情及实测范围见 [模型服务接入](docs/engineering/模型服务接入_2026-09-14.md)。

当前为用户主动触发的有限上下文分析，尚未替代后台全历史规则处理，跨会话及跨渠道自动归并仍待接通。历史本地模型评测（不是当前默认接入方式）与限制见 [AI 事项分析验收](docs/evals/AI事项分析验收.md)。

## AI 任务聊天与编排

左侧「AI 聊天」选择项目后，可自然语言查询、新增、修改任务，或删除到可恢复回收站。AI 先展示变更预览，确认后原子提交，保护人工修改与重复确认。模型沿用 Responses / Chat Completions 或已登录 CLI 配置；处理过程默认折叠。连接页的事项分析现支持已同步、授权有效的飞书会话。

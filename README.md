...
- SQLite v1 迁移、WAL/FULL、外键、事件/作业/游标事务和修订去重。
- JSON Schema 输入契约及派生类型；领域、应用与适配器接口分层。

飞书、AI 会话、GitHub、模型与插件执行尚未接入；“示例体验”提供虚构事项，支持筛选、搜索、新增及可撤销的状态预览，仅保存在窗口内存；“我的工作区”显示真实未接入空态。条件核验、完整状态机、作业调度、人工编辑及加密数据库仍待开发。当前 SQLite 仅用于测试数据。
...
目前事件接收仅供集成测试；调用方须先授权并登记来源，同一来源分页串行提交。数据库登记不代替正式授权模块。迁移 v1 建立的是最小表结构，尚不包含完整条件/证据/人工决定模型，也没有后台作业消费器。

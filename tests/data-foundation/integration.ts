...
async function main(): Promise<void> {
  ...
  await test('merge: foundation refuses production v16 without changing data', env => {
  ...
  await test('merge: production v2 is distinguishable and still upgrades to v16', env => {
    seedLegacy(env.path)
    ...
    assert.equal(upgraded.health().schemaVersion, 16)
    upgraded.close()
  })

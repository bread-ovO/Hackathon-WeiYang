  ...
  https.request = (
    ...
    const body = {
      ...
      data: {
        ...
        items: [
          {
            message_id: page ? 'om_change' : 'om_human',
            ...(page ? { parent_id: 'om_human' } : {}),
            chat_id: 'oc_desktop',
            create_time: String(start + (page ? 2000 : 1000)),
            update_time: String(start + (page ? 2000 : 1000)),
            sender: {
              sender_type: 'user',
              id_type: 'open_id',
              id: 'ou_fictional_planner',
            },
            deleted: false,
            body: {
              content: JSON.stringify({
                text: page
                  ? '截止时间改为 2026-09-20T18:00:00+08:00'
                  : '我会完成周报验收',
              }),
            },
          },

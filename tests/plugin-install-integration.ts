...
try {
  ...
  const largeCursor = JSON.stringify({
    seen: Array.from({ length: 128 }, (_, i) =>
  ...
  const batch = (cursor: string, expectedCursor: string, externalId: string) =>
    store.plugins.receiveBatch({
      id: installed.id,
      ...
      expectedCursor,
      events: [
        { ...event(externalId), sourceInstanceId: current.sourceInstanceId },
      ],
    })
  assert.equal(
    batch(largeCursor, current.cursor, 'large-cursor-page').inserted,
    1,
  )
  ...
  assert.equal(
    batch(maximumCursor, largeCursor, 'maximum-cursor-page').inserted,
    1,
  )
  assert.throws(
    () => batch('c'.repeat(16385), maximumCursor, 'oversize-cursor-page'),
    /PLUGIN_INVALID_DATA/,
  )
  ...
  assert.equal(reopen.health().schemaVersion, 22)
  reopen.close()
  console.log(

...
test('GitHub form verifies scope using vault and actual transport, persists PR observations and fences credential removal', async () => {
  test.setTimeout(90000)
  const root = await realpath(
      await mkdtemp(join(tmpdir(), 'bugu-github-connect-')),
    ),
    profile = join(root, 'profile'),
    fixture = join(root, 'network.cjs'),
    token = 'fictional-desktop-github-token',
    tokenFile = join(root, 'token.txt')
  await writeFile(tokenFile, token + '\n')
  await build({
    entryPoints: [resolve('tests/fixtures/github-network.ts')],
    outfile: fixture,
    bundle: true,
    platform: 'node',
    format: 'cjs',
  ...
  try {
    ...
    const projectId = await page.evaluate(async () => {
      const result =
        await window.memo.workspace.createProject('GitHub真实配置验收')
    ...
    await panel.getByLabel('GitHub仓库名').fill('desktop')
    await panel.getByLabel('GitHub读取凭据').selectOption(credentialId)
    await panel.getByRole('button', { name: '验证并启用仓库' }).click()
    await expect
      .poll(async () => {
        const r = await page.evaluate(() => window.memo.github.list())
    ...
    await expect
      ...
      .toBe(1)
    await panel.getByRole('button', { name: '刷新仓库连接' }).click()
    await panel.getByRole('button', { name: '查看 PR 观察记录' }).click()
    await expect(panel.getByText('合成 GitHub 交付记录')).toBeVisible()
    await expect(
      panel.getByText('观测状态：已合并', { exact: true }),
    ).toBeVisible()
    const snapshot = await page.evaluate(() => window.memo.github.list())
    if (!snapshot.ok) throw Error('LIST_FAILED')
    const connection = snapshot.data.connections[0]!
    expect(connection.repositoryId).toBe(987)
    ...
    await page.screenshot({ path: 'test-results/github-connected-wide.png' })
    expect(
    ...
    const after = await page.evaluate(() => window.memo.github.list())
    ...
    expect(
      stats.paths.filter((p) => p.includes('/pulls?')).length,

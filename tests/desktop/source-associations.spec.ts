...
test('manual task source picker and direct identity mapping fence real plan confirmation and preserve drafts', async () => {
  test.setTimeout(90000)
  const root = await realpath(
      await mkdtemp(join(tmpdir(), 'bugu-source-identity-')),
    ),
  ...
  await build({
    entryPoints: [resolve('tests/fixtures/source-identity-network.ts')],
    outfile: fixture,
    bundle: true,
    platform: 'node',
    format: 'cjs',
  ...
  try {
    ...
    const targetId = await page.evaluate(
      async ({ projectId, credentialId, startTime }) => {
        const connected = await window.memo.feishu.connect({
          projectId,
          credentialId,
          chatId: 'oc_identity_b',
          startTime,
        })
        if (!connected.ok) throw Error('SECOND_CONNECTION_FAILED')
        const created = await window.memo.workspace.createTask(
          projectId,
          '人工关联验收事项',
        )
        if (!created.ok) throw Error('CREATE_TASK_FAILED')
        return created.data.tasks.find((t) => t.title === '人工关联验收事项')!
          .id
      },
      { projectId, credentialId, startTime: historyStart.getTime() },
    )
    await expect
      .poll(
        async () => {
          const r = await page.evaluate(
            (projectId) => window.memo.workspace.sourceEvents({ projectId }),
            projectId,
          )
          return r.ok ? r.data.events.length : 0
        },
        { timeout: 30000 },
      )
      .toBe(3)
    ...
    await detail
      ...
      .fill('保留身份确认时的条件草稿')
    await detail.locator('.source-associations > summary').click()
    const associations = detail.getByRole('region', {
      name: '来源关联与身份确认',
    })
    await associations
      .getByRole('button', { name: '选择已收录来源记录' })
      .click()
    await associations
      .getByLabel('来源关联操作理由')
      .fill('已核实来源对象属于此人工事项')
    await associations
      .getByRole('radio', { name: '选择记录 第一个来源中的交付讨论记录' })
      .check()
    await associations.getByRole('button', { name: '确认关联当前事项' }).click()
    await expect(
      associations.getByText('已关联此来源对象，事项字段保持不变。'),
    ).toBeVisible()
    await associations.getByRole('button', { name: '作为身份左端' }).click()
    await associations
      .getByRole('radio', { name: '选择记录 第二个来源中的交付讨论记录' })
      .check()
    await associations.getByRole('button', { name: '确认关联当前事项' }).click()
    await expect
      .poll(async () => {
        const r = await page.evaluate(
          (scope) => window.memo.workspace.sourceBindings(scope),
          { projectId, taskId: targetId },
        )
        return r.ok ? r.data.bindings.length : 0
      })
      .toBe(2)
    await associations.getByRole('button', { name: '作为身份右端' }).click()
    await associations
      .getByRole('radio', {
        name: '选择记录 截止时间改为 2026-09-25T18:00:00+08:00',
      })
      .check()
    await associations.getByRole('button', { name: '重新评估所选记录' }).click()
    const plans = detail.getByRole('region', { name: '计划变更建议' })
    await expect(
      plans.getByText('发送者与原始承诺不同，需要进一步核实关联'),
    ).toBeVisible()
    await associations.getByRole('button', { name: '确认两端身份映射' }).click()
    await expect(
      associations.getByText('映射有效', { exact: true }),
    ).toBeVisible()
    await associations.getByRole('button', { name: '重新评估所选记录' }).click()
    await expect(
      plans.getByRole('button', { name: '核实并改期' }),
    ).toBeVisible()
    await associations.getByRole('button', { name: '撤销此身份映射' }).click()
    await associations
      .getByRole('button', { name: '确认撤销', exact: true })
      .click()
    await expect(
      plans.getByText('身份映射已撤销或变化，不能直接应用'),
    ).toBeVisible()
    await expect(plans.getByRole('button', { name: '核实并改期' })).toHaveCount(
      0,
    )
    await associations.getByRole('button', { name: '确认两端身份映射' }).click()
    await associations.getByRole('button', { name: '重新评估所选记录' }).click()
    ...
    expect(saved.ok && saved.data.task.dueAt).toBe('2026-09-25T10:00:00.000Z')
    await associations.scrollIntoViewIfNeeded()
    await page.screenshot({ path: 'test-results/source-identity-wide.png' })
    await app.evaluate(({ BrowserWindow }) =>
      BrowserWindow.getAllWindows()[0]!.setSize(900, 720),
    )
    await associations.scrollIntoViewIfNeeded()
    await page.screenshot({ path: 'test-results/source-identity-narrow.png' })
  } finally {
    await app.close()

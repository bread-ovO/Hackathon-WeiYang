...
const pull = {
  number: 1,
  title: 'Fictional integration PR',
...
const nextLink =
  '<https://api.github.com/repos/org/project/pulls?state=all&per_page=100&sort=created&direction=asc&page=2>; rel="next"'
const github = () =>
  createGithubPullRequestsFetcher(
    token,
    'org',
...
const feishu = () =>
  new FeishuHistoryAdapter(
    'feishu:fictional-chat',
    createFeishuMessagesFetcher(
      token,
      'oc_fictional',
      createSourceHttpTransport(),
...
function options(index: number): RequestOptions {
  return network.request.mock.calls[index]![0]
}
function headers(index: number): Record<string, string> {
  return options(index).headers as Record<string, string>
}
function header(index: number, key: string) {
  return Object.entries(headers(index)).find(
...
describe('real source adapter → host HTTP capability → HTTPS transport', () => {
  it('GitHub 200 then ETag conditional 304 reuses validated events and Link cursor', async () => {
    plan.push(
      ...
      { status: 304, headers: { etag: '"revision-1"' } },
    )
    const fetch = github()
    const first = await fetch('', signal())
    const second = await fetch('', signal())
    ...
    expect(JSON.parse(second.events[0]!.text)).toMatchObject({
      state: 'merged',
      base: { ref: 'main' },
      head: { ref: 'feature' },
    })
    ...
    options(0).lookup!('api.github.com', {}, lookupCallback)
  ...
  it('GitHub follows only the validated next page and does not forward page-one ETag', async () => {
    ...
    const fetch = github()
    const first = await fetch('', signal())
    const second = await fetch(first.nextCursor, signal())
    expect(options(1).hostname).toBe('api.github.com')
    expect(
      new URL(`https://api.github.com${options(1).path}`).searchParams.get(
        'page',
      ),
    ).toBe('2')
  ...
  it.each([429, 403])(
    'GitHub %s Retry-After survives the real transport boundary without body leakage',
    async (status) => {
      plan.push({
        status,
        headers: { 'content-type': 'text/html', 'retry-after': '90' },
        body: { message: `do not expose ${token}` },
      })
  ...
  it.each([401, 403, 404])(
    'GitHub %s auth failure survives byte transport and scrubs message',
    async (status) => {
      plan.push({ status, body: { message: `private ${token}` } })
      await expect(github()('', signal())).rejects.toThrow('GITHUB_AUTH_FAILED')
    },
  )
  it('GitHub secondary limit survives only as a fixed hint across byte transport', async () => {
    plan.push({
      status: 403,
      body: { message: `You have exceeded a secondary rate limit. ${token}` },
    })
    const error = await github()('', signal()).catch((e) => e)
    expect(error).toBeInstanceOf(GithubRateLimitError)
  ...
  it('Feishu uses its fixed host, converts milliseconds, and emits app messages as assistant', async () => {
    plan.push({
      body: {
        ...
        data: {
          items: [
            {
              message_id: 'om_fictional',
              create_time: '1700000000000',
              update_time: '1700000001000',
              sender: { sender_type: 'app', id: 'bot' },
              body: {
                content: JSON.stringify({ text: 'Fictional source message' }),
              },
            },
          ],
          has_more: false,
    ...
    expect(result.events[0]).toMatchObject({
      sourceInstanceId: 'feishu:fictional-chat',
      externalId: 'om_fictional',
      occurredAt: '2023-11-14T22:13:20.000Z',
      revision: '1700000001000',
      role: 'assistant',
      text: 'Fictional source message',
    })
    expect(JSON.stringify(result)).not.toContain(token)
    expect(options(0).path).not.toContain(token)
  })
  it.each(['github', 'feishu'])(
    '%s rejects invalid UTF8 through the real byte decoder',
    async (kind) => {
      plan.push({ bytes: Uint8Array.from([0xff]) })
      const pending =
        kind === 'github' ? github()('', signal()) : feishu().pull('', signal())
  ...
  it('rejects a 304 response carrying bytes instead of reusing cached events', async () => {
    plan.push(
      ...
      { status: 304, headers: {}, bytes: Buffer.from('unexpected') },
    )
    const fetch = github()
    await fetch('', signal())
  ...
  it.each(['github', 'feishu'])(
    '%s rejects private DNS before opening a socket',
    async (kind) => {
  ...
  it.each(['github', 'feishu'])(
    '%s rejects redirects without forwarding the token',
    async (kind) => {
      plan.push({
        status: 302,
  ...
  it.each(['github', 'feishu'])(
    '%s cancellation destroys the socket and yields no events',
    async (kind) => {
      plan.push({ pause: true })
      const controller = new AbortController()
      const pending =
        kind === 'github'
          ? github()('', controller.signal)
          : feishu().pull('', controller.signal)
      const rejection = expect(pending).rejects.toThrow(
        kind === 'github' ? 'GITHUB_CANCELLED' : 'FEISHU_ABORTED',
      )

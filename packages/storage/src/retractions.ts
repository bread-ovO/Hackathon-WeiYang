...
export function createRetractions(db: Database.Database) {
  ...
  function observe(projectId: string, eventId: number) {
    const e = db
      ...
      EXISTS(SELECT 1 FROM plugin_bindings p WHERE p.source_instance_id=e.source_id AND p.project_id=ep.project_id AND p.enabled=1 AND p.uninstalled=0) OR EXISTS(SELECT 1 FROM github_connections h WHERE h.source_id=e.source_id AND h.project_id=ep.project_id AND h.enabled=1 AND h.revoked=0) OR EXISTS(SELECT 1 FROM feishu_connections f WHERE f.source_id=e.source_id AND f.project_id=ep.project_id AND f.enabled=1 AND f.revoked=0))`,
      )
